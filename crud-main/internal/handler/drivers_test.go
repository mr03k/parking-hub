...
func newDriversHandler() *gotest.Handler {
	...
	d.IDao = dao.NewDriversDao(d.DB, c.ICache.(cache.DriversCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &driversHandler{iDao: d.IDao.(dao.DriversDao)}
	iHandler := h.IHandler.(DriversHandler)
...
func Test_driversHandler_Create(t *testing.T) {
	h := newDriversHandler()
	defer h.Close()
	testData := &types.CreateDriversRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Drivers))
...
func Test_driversHandler_DeleteByID(t *testing.T) {
	h := newDriversHandler()
	defer h.Close()
	testData := h.TestData.(*model.Drivers)
...
func Test_driversHandler_UpdateByID(t *testing.T) {
	h := newDriversHandler()
	defer h.Close()
	testData := &types.UpdateDriversByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Drivers))
...
func Test_driversHandler_GetByID(t *testing.T) {
	h := newDriversHandler()
	defer h.Close()
	testData := h.TestData.(*model.Drivers)
...
func Test_driversHandler_List(t *testing.T) {
	h := newDriversHandler()
	defer h.Close()
	testData := h.TestData.(*model.Drivers)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListDriverssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewDriversHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewDriversHandler()
