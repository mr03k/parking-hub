...
func newPeakHourMultipliersHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPeakHourMultipliersDao(d.DB, c.ICache.(cache.PeakHourMultipliersCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &peakHourMultipliersHandler{iDao: d.IDao.(dao.PeakHourMultipliersDao)}
	iHandler := h.IHandler.(PeakHourMultipliersHandler)

	testFns := []gotest.RouterInfo{
		{
			FuncName:    "Create",
			Method:      http.MethodPost,
			Path:        "/peakHourMultipliers",
			HandlerFunc: iHandler.Create,
		},
		{
			FuncName:    "DeleteByID",
			Method:      http.MethodDelete,
			Path:        "/peakHourMultipliers/:id",
...
func Test_peakHourMultipliersHandler_Create(t *testing.T) {
	h := newPeakHourMultipliersHandler()
	defer h.Close()
	testData := &types.CreatePeakHourMultipliersRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.PeakHourMultipliers))
...
func Test_peakHourMultipliersHandler_DeleteByID(t *testing.T) {
	h := newPeakHourMultipliersHandler()
	defer h.Close()
	testData := h.TestData.(*model.PeakHourMultipliers)
...
func Test_peakHourMultipliersHandler_UpdateByID(t *testing.T) {
	h := newPeakHourMultipliersHandler()
	defer h.Close()
	testData := &types.UpdatePeakHourMultipliersByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.PeakHourMultipliers))
...
func Test_peakHourMultipliersHandler_GetByID(t *testing.T) {
	h := newPeakHourMultipliersHandler()
	defer h.Close()
	testData := h.TestData.(*model.PeakHourMultipliers)
...
func Test_peakHourMultipliersHandler_List(t *testing.T) {
	h := newPeakHourMultipliersHandler()
	defer h.Close()
	testData := h.TestData.(*model.PeakHourMultipliers)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPeakHourMultiplierssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPeakHourMultipliersHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPeakHourMultipliersHandler()
}
