...
func newCalendarsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewCalendarsDao(d.DB, c.ICache.(cache.CalendarsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &calendarsHandler{iDao: d.IDao.(dao.CalendarsDao)}
	iHandler := h.IHandler.(CalendarsHandler)
...
func Test_calendarsHandler_Create(t *testing.T) {
	h := newCalendarsHandler()
	defer h.Close()
	testData := &types.CreateCalendarsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Calendars))
...
func Test_calendarsHandler_DeleteByID(t *testing.T) {
	h := newCalendarsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Calendars)
...
func Test_calendarsHandler_UpdateByID(t *testing.T) {
	h := newCalendarsHandler()
	defer h.Close()
	testData := &types.UpdateCalendarsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Calendars))
...
func Test_calendarsHandler_GetByID(t *testing.T) {
	h := newCalendarsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Calendars)
...
func Test_calendarsHandler_List(t *testing.T) {
	h := newCalendarsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Calendars)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListCalendarssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewCalendarsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewCalendarsHandler()
}
