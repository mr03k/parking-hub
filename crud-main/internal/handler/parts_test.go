...
func newPartsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPartsDao(d.DB, c.ICache.(cache.PartsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &partsHandler{iDao: d.IDao.(dao.PartsDao)}
	iHandler := h.IHandler.(PartsHandler)
...
func Test_partsHandler_Create(t *testing.T) {
	h := newPartsHandler()
	defer h.Close()
	testData := &types.CreatePartsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Parts))
...
func Test_partsHandler_DeleteByID(t *testing.T) {
	h := newPartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parts)
...
func Test_partsHandler_UpdateByID(t *testing.T) {
	h := newPartsHandler()
	defer h.Close()
	testData := &types.UpdatePartsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Parts))
...
func Test_partsHandler_GetByID(t *testing.T) {
	h := newPartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parts)
...
func Test_partsHandler_List(t *testing.T) {
	h := newPartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parts)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPartssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewPartsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewPartsHandler()
