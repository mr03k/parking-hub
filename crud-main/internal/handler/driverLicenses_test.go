...
func newDriverLicensesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewDriverLicensesDao(d.DB, c.ICache.(cache.DriverLicensesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &driverLicensesHandler{iDao: d.IDao.(dao.DriverLicensesDao)}
	iHandler := h.IHandler.(DriverLicensesHandler)
...
func Test_driverLicensesHandler_Create(t *testing.T) {
	h := newDriverLicensesHandler()
	defer h.Close()
	testData := &types.CreateDriverLicensesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.DriverLicenses))
...
func Test_driverLicensesHandler_DeleteByID(t *testing.T) {
	h := newDriverLicensesHandler()
	defer h.Close()
	testData := h.TestData.(*model.DriverLicenses)
...
func Test_driverLicensesHandler_UpdateByID(t *testing.T) {
	h := newDriverLicensesHandler()
	defer h.Close()
	testData := &types.UpdateDriverLicensesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.DriverLicenses))
...
func Test_driverLicensesHandler_GetByID(t *testing.T) {
	h := newDriverLicensesHandler()
	defer h.Close()
	testData := h.TestData.(*model.DriverLicenses)
...
func Test_driverLicensesHandler_List(t *testing.T) {
	h := newDriverLicensesHandler()
	defer h.Close()
	testData := h.TestData.(*model.DriverLicenses)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListDriverLicensessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewDriverLicensesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewDriverLicensesHandler()
}
