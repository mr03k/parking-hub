...
func newParkingsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewParkingsDao(d.DB, c.ICache.(cache.ParkingsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &parkingsHandler{iDao: d.IDao.(dao.ParkingsDao)}
	iHandler := h.IHandler.(ParkingsHandler)
...
func Test_parkingsHandler_Create(t *testing.T) {
	h := newParkingsHandler()
	defer h.Close()
	testData := &types.CreateParkingsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Parkings))
...
func Test_parkingsHandler_DeleteByID(t *testing.T) {
	h := newParkingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parkings)
...
func Test_parkingsHandler_UpdateByID(t *testing.T) {
	h := newParkingsHandler()
	defer h.Close()
	testData := &types.UpdateParkingsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Parkings))
...
func Test_parkingsHandler_GetByID(t *testing.T) {
	h := newParkingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parkings)
...
func Test_parkingsHandler_List(t *testing.T) {
	h := newParkingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Parkings)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListParkingssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewParkingsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewParkingsHandler()
}
