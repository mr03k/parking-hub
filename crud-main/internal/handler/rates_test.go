...
func newRatesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewRatesDao(d.DB, c.ICache.(cache.RatesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &ratesHandler{iDao: d.IDao.(dao.RatesDao)}
	iHandler := h.IHandler.(RatesHandler)
...
func Test_ratesHandler_Create(t *testing.T) {
	h := newRatesHandler()
	defer h.Close()
	testData := &types.CreateRatesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Rates))
...
func Test_ratesHandler_DeleteByID(t *testing.T) {
	h := newRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rates)
...
func Test_ratesHandler_UpdateByID(t *testing.T) {
	h := newRatesHandler()
	defer h.Close()
	testData := &types.UpdateRatesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Rates))
...
func Test_ratesHandler_GetByID(t *testing.T) {
	h := newRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rates)
...
func Test_ratesHandler_List(t *testing.T) {
	h := newRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rates)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListRatessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewRatesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewRatesHandler()
