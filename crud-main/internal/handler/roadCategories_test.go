...
func newRoadCategoriesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewRoadCategoriesDao(d.DB, c.ICache.(cache.RoadCategoriesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &roadCategoriesHandler{iDao: d.IDao.(dao.RoadCategoriesDao)}
	iHandler := h.IHandler.(RoadCategoriesHandler)
...
func Test_roadCategoriesHandler_Create(t *testing.T) {
	h := newRoadCategoriesHandler()
	defer h.Close()
	testData := &types.CreateRoadCategoriesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.RoadCategories))
...
func Test_roadCategoriesHandler_DeleteByID(t *testing.T) {
	h := newRoadCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.RoadCategories)
...
func Test_roadCategoriesHandler_UpdateByID(t *testing.T) {
	h := newRoadCategoriesHandler()
	defer h.Close()
	testData := &types.UpdateRoadCategoriesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.RoadCategories))
...
func Test_roadCategoriesHandler_GetByID(t *testing.T) {
	h := newRoadCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.RoadCategories)
...
func Test_roadCategoriesHandler_List(t *testing.T) {
	h := newRoadCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.RoadCategories)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListRoadCategoriessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewRoadCategoriesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewRoadCategoriesHandler()
}
