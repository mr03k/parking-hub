...
func newDistrictsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewDistrictsDao(d.DB, c.ICache.(cache.DistrictsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &districtsHandler{iDao: d.IDao.(dao.DistrictsDao)}
	iHandler := h.IHandler.(DistrictsHandler)
...
func Test_districtsHandler_Create(t *testing.T) {
	h := newDistrictsHandler()
	defer h.Close()
	testData := &types.CreateDistrictsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Districts))
...
func Test_districtsHandler_DeleteByID(t *testing.T) {
	h := newDistrictsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Districts)
...
func Test_districtsHandler_UpdateByID(t *testing.T) {
	h := newDistrictsHandler()
	defer h.Close()
	testData := &types.UpdateDistrictsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Districts))
...
func Test_districtsHandler_GetByID(t *testing.T) {
	h := newDistrictsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Districts)
...
func Test_districtsHandler_List(t *testing.T) {
	h := newDistrictsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Districts)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListDistrictssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewDistrictsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewDistrictsHandler()
}
