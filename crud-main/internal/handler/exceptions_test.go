...
func newExceptionsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewExceptionsDao(d.DB, c.ICache.(cache.ExceptionsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &exceptionsHandler{iDao: d.IDao.(dao.ExceptionsDao)}
	iHandler := h.IHandler.(ExceptionsHandler)
...
func Test_exceptionsHandler_Create(t *testing.T) {
	h := newExceptionsHandler()
	defer h.Close()
	testData := &types.CreateExceptionsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Exceptions))
...
func Test_exceptionsHandler_DeleteByID(t *testing.T) {
	h := newExceptionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Exceptions)
...
func Test_exceptionsHandler_UpdateByID(t *testing.T) {
	h := newExceptionsHandler()
	defer h.Close()
	testData := &types.UpdateExceptionsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Exceptions))
...
func Test_exceptionsHandler_GetByID(t *testing.T) {
	h := newExceptionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Exceptions)
...
func Test_exceptionsHandler_List(t *testing.T) {
	h := newExceptionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Exceptions)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListExceptionssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewExceptionsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewExceptionsHandler()
}
