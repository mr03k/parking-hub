...
func newBaseRatesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewBaseRatesDao(d.DB, c.ICache.(cache.BaseRatesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &baseRatesHandler{iDao: d.IDao.(dao.BaseRatesDao)}
	iHandler := h.IHandler.(BaseRatesHandler)
...
func Test_baseRatesHandler_Create(t *testing.T) {
	h := newBaseRatesHandler()
	defer h.Close()
	testData := &types.CreateBaseRatesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.BaseRates))
...
func Test_baseRatesHandler_DeleteByID(t *testing.T) {
	h := newBaseRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.BaseRates)
...
func Test_baseRatesHandler_UpdateByID(t *testing.T) {
	h := newBaseRatesHandler()
	defer h.Close()
	testData := &types.UpdateBaseRatesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.BaseRates))
...
func Test_baseRatesHandler_GetByID(t *testing.T) {
	h := newBaseRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.BaseRates)
...
func Test_baseRatesHandler_List(t *testing.T) {
	h := newBaseRatesHandler()
	defer h.Close()
	testData := h.TestData.(*model.BaseRates)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListBaseRatessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewBaseRatesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewBaseRatesHandler()
}
