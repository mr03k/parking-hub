...
func newAssignmentsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewAssignmentsDao(d.DB, c.ICache.(cache.AssignmentsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &assignmentsHandler{iDao: d.IDao.(dao.AssignmentsDao)}
	iHandler := h.IHandler.(AssignmentsHandler)
...
func Test_assignmentsHandler_Create(t *testing.T) {
	h := newAssignmentsHandler()
	defer h.Close()
	testData := &types.CreateAssignmentsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Assignments))
...
func Test_assignmentsHandler_DeleteByID(t *testing.T) {
	h := newAssignmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Assignments)
...
func Test_assignmentsHandler_UpdateByID(t *testing.T) {
	h := newAssignmentsHandler()
	defer h.Close()
	testData := &types.UpdateAssignmentsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Assignments))
...
func Test_assignmentsHandler_GetByID(t *testing.T) {
	h := newAssignmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Assignments)
...
func Test_assignmentsHandler_List(t *testing.T) {
	h := newAssignmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Assignments)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListAssignmentssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewAssignmentsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewAssignmentsHandler()
}
