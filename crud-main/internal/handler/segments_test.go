...
func newSegmentsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewSegmentsDao(d.DB, c.ICache.(cache.SegmentsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &segmentsHandler{iDao: d.IDao.(dao.SegmentsDao)}
	iHandler := h.IHandler.(SegmentsHandler)
...
func Test_segmentsHandler_Create(t *testing.T) {
	h := newSegmentsHandler()
	defer h.Close()
	testData := &types.CreateSegmentsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Segments))
...
func Test_segmentsHandler_DeleteByID(t *testing.T) {
	h := newSegmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Segments)
...
func Test_segmentsHandler_UpdateByID(t *testing.T) {
	h := newSegmentsHandler()
	defer h.Close()
	testData := &types.UpdateSegmentsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Segments))
...
func Test_segmentsHandler_GetByID(t *testing.T) {
	h := newSegmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Segments)
...
func Test_segmentsHandler_List(t *testing.T) {
	h := newSegmentsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Segments)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListSegmentssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewSegmentsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewSegmentsHandler()
}
