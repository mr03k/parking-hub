...
func newVehicleCategoriesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewVehicleCategoriesDao(d.DB, c.ICache.(cache.VehicleCategoriesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &vehicleCategoriesHandler{iDao: d.IDao.(dao.VehicleCategoriesDao)}
	iHandler := h.IHandler.(VehicleCategoriesHandler)
...
func Test_vehicleCategoriesHandler_Create(t *testing.T) {
	h := newVehicleCategoriesHandler()
	defer h.Close()
	testData := &types.CreateVehicleCategoriesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.VehicleCategories))
...
func Test_vehicleCategoriesHandler_DeleteByID(t *testing.T) {
	h := newVehicleCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.VehicleCategories)
...
func Test_vehicleCategoriesHandler_UpdateByID(t *testing.T) {
	h := newVehicleCategoriesHandler()
	defer h.Close()
	testData := &types.UpdateVehicleCategoriesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.VehicleCategories))
...
func Test_vehicleCategoriesHandler_GetByID(t *testing.T) {
	h := newVehicleCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.VehicleCategories)
...
func Test_vehicleCategoriesHandler_List(t *testing.T) {
	h := newVehicleCategoriesHandler()
	defer h.Close()
	testData := h.TestData.(*model.VehicleCategories)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListVehicleCategoriessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewVehicleCategoriesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewVehicleCategoriesHandler()
}
