...
func newCitiesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewCitiesDao(d.DB, c.ICache.(cache.CitiesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &citiesHandler{iDao: d.IDao.(dao.CitiesDao)}
	iHandler := h.IHandler.(CitiesHandler)
...
func Test_citiesHandler_Create(t *testing.T) {
	h := newCitiesHandler()
	defer h.Close()
	testData := &types.CreateCitiesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Cities))
...
func Test_citiesHandler_DeleteByID(t *testing.T) {
	h := newCitiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Cities)
...
func Test_citiesHandler_UpdateByID(t *testing.T) {
	h := newCitiesHandler()
	defer h.Close()
	testData := &types.UpdateCitiesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Cities))
...
func Test_citiesHandler_GetByID(t *testing.T) {
	h := newCitiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Cities)
...
func Test_citiesHandler_List(t *testing.T) {
	h := newCitiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Cities)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListCitiessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewCitiesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewCitiesHandler()
