...
func newContractsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewContractsDao(d.DB, c.ICache.(cache.ContractsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &contractsHandler{iDao: d.IDao.(dao.ContractsDao)}
	iHandler := h.IHandler.(ContractsHandler)
...
func Test_contractsHandler_Create(t *testing.T) {
	h := newContractsHandler()
	defer h.Close()
	testData := &types.CreateContractsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Contracts))
...
func Test_contractsHandler_DeleteByID(t *testing.T) {
	h := newContractsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Contracts)
...
func Test_contractsHandler_UpdateByID(t *testing.T) {
	h := newContractsHandler()
	defer h.Close()
	testData := &types.UpdateContractsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Contracts))
...
func Test_contractsHandler_GetByID(t *testing.T) {
	h := newContractsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Contracts)
...
func Test_contractsHandler_List(t *testing.T) {
	h := newContractsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Contracts)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListContractssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewContractsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewContractsHandler()
}
