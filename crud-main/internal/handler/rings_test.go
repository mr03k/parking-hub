...
func newRingsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewRingsDao(d.DB, c.ICache.(cache.RingsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &ringsHandler{iDao: d.IDao.(dao.RingsDao)}
	iHandler := h.IHandler.(RingsHandler)
...
func Test_ringsHandler_Create(t *testing.T) {
	h := newRingsHandler()
	defer h.Close()
	testData := &types.CreateRingsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Rings))
...
func Test_ringsHandler_DeleteByID(t *testing.T) {
	h := newRingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rings)
...
func Test_ringsHandler_UpdateByID(t *testing.T) {
	h := newRingsHandler()
	defer h.Close()
	testData := &types.UpdateRingsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Rings))
...
func Test_ringsHandler_GetByID(t *testing.T) {
	h := newRingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rings)
...
func Test_ringsHandler_List(t *testing.T) {
	h := newRingsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Rings)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListRingssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewRingsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewRingsHandler()
