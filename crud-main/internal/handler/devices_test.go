...
func newDevicesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewDevicesDao(d.DB, c.ICache.(cache.DevicesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &devicesHandler{iDao: d.IDao.(dao.DevicesDao)}
	iHandler := h.IHandler.(DevicesHandler)
...
func Test_devicesHandler_Create(t *testing.T) {
	h := newDevicesHandler()
	defer h.Close()
	testData := &types.CreateDevicesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Devices))
...
func Test_devicesHandler_DeleteByID(t *testing.T) {
	h := newDevicesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Devices)
...
func Test_devicesHandler_UpdateByID(t *testing.T) {
	h := newDevicesHandler()
	defer h.Close()
	testData := &types.UpdateDevicesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Devices))
...
func Test_devicesHandler_GetByID(t *testing.T) {
	h := newDevicesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Devices)
...
func Test_devicesHandler_List(t *testing.T) {
	h := newDevicesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Devices)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListDevicessRequest{query.Params{
		Page:  0,
		Limit: 10,
