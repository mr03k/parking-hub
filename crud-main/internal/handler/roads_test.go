...
func newRoadsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewRoadsDao(d.DB, c.ICache.(cache.RoadsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &roadsHandler{iDao: d.IDao.(dao.RoadsDao)}
	iHandler := h.IHandler.(RoadsHandler)
...
func Test_roadsHandler_Create(t *testing.T) {
	h := newRoadsHandler()
	defer h.Close()
	testData := &types.CreateRoadsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Roads))
...
func Test_roadsHandler_DeleteByID(t *testing.T) {
	h := newRoadsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Roads)
...
func Test_roadsHandler_UpdateByID(t *testing.T) {
	h := newRoadsHandler()
	defer h.Close()
	testData := &types.UpdateRoadsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Roads))
...
func Test_roadsHandler_GetByID(t *testing.T) {
	h := newRoadsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Roads)
...
func Test_roadsHandler_List(t *testing.T) {
	h := newRoadsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Roads)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListRoadssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewRoadsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewRoadsHandler()
