...
func newFormsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewFormsDao(d.DB, c.ICache.(cache.FormsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &formsHandler{iDao: d.IDao.(dao.FormsDao)}
	iHandler := h.IHandler.(FormsHandler)
...
func Test_formsHandler_Create(t *testing.T) {
	h := newFormsHandler()
	defer h.Close()
	testData := &types.CreateFormsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Forms))
...
func Test_formsHandler_DeleteByID(t *testing.T) {
	h := newFormsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Forms)
...
func Test_formsHandler_UpdateByID(t *testing.T) {
	h := newFormsHandler()
	defer h.Close()
	testData := &types.UpdateFormsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Forms))
...
func Test_formsHandler_GetByID(t *testing.T) {
	h := newFormsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Forms)
...
func Test_formsHandler_List(t *testing.T) {
	h := newFormsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Forms)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListFormssRequest{query.Params{
		Page:  0,
		Limit: 10,
