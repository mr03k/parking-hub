...
func newModulesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewModulesDao(d.DB, c.ICache.(cache.ModulesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &modulesHandler{iDao: d.IDao.(dao.ModulesDao)}
	iHandler := h.IHandler.(ModulesHandler)
...
func Test_modulesHandler_Create(t *testing.T) {
	h := newModulesHandler()
	defer h.Close()
	testData := &types.CreateModulesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Modules))
...
func Test_modulesHandler_DeleteByID(t *testing.T) {
	h := newModulesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Modules)
...
func Test_modulesHandler_UpdateByID(t *testing.T) {
	h := newModulesHandler()
	defer h.Close()
	testData := &types.UpdateModulesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Modules))
...
func Test_modulesHandler_GetByID(t *testing.T) {
	h := newModulesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Modules)
...
func Test_modulesHandler_List(t *testing.T) {
	h := newModulesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Modules)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListModulessRequest{query.Params{
		Page:  0,
		Limit: 10,
