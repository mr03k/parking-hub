...
func newDevicePartsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewDevicePartsDao(d.DB, c.ICache.(cache.DevicePartsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &devicePartsHandler{iDao: d.IDao.(dao.DevicePartsDao)}
	iHandler := h.IHandler.(DevicePartsHandler)
...
func Test_devicePartsHandler_Create(t *testing.T) {
	h := newDevicePartsHandler()
	defer h.Close()
	testData := &types.CreateDevicePartsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.DeviceParts))
...
func Test_devicePartsHandler_DeleteByID(t *testing.T) {
	h := newDevicePartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.DeviceParts)
...
func Test_devicePartsHandler_UpdateByID(t *testing.T) {
	h := newDevicePartsHandler()
	defer h.Close()
	testData := &types.UpdateDevicePartsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.DeviceParts))
...
func Test_devicePartsHandler_GetByID(t *testing.T) {
	h := newDevicePartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.DeviceParts)
...
func Test_devicePartsHandler_List(t *testing.T) {
	h := newDevicePartsHandler()
	defer h.Close()
	testData := h.TestData.(*model.DeviceParts)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListDevicePartssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewDevicePartsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewDevicePartsHandler()
}
