...
func newVehiclesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewVehiclesDao(d.DB, c.ICache.(cache.VehiclesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &vehiclesHandler{iDao: d.IDao.(dao.VehiclesDao)}
	iHandler := h.IHandler.(VehiclesHandler)
...
func Test_vehiclesHandler_Create(t *testing.T) {
	h := newVehiclesHandler()
	defer h.Close()
	testData := &types.CreateVehiclesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Vehicles))
...
func Test_vehiclesHandler_DeleteByID(t *testing.T) {
	h := newVehiclesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Vehicles)
...
func Test_vehiclesHandler_UpdateByID(t *testing.T) {
	h := newVehiclesHandler()
	defer h.Close()
	testData := &types.UpdateVehiclesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Vehicles))
...
func Test_vehiclesHandler_GetByID(t *testing.T) {
	h := newVehiclesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Vehicles)
...
func Test_vehiclesHandler_List(t *testing.T) {
	h := newVehiclesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Vehicles)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListVehiclessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewVehiclesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewVehiclesHandler()
}
