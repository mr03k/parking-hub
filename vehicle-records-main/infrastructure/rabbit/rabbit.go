...
func (cr *ConsumerRunner) Setup(ch *amqp091.Channel, rch *amqp091.Channel) error {
	cr.channel = ch
	cr.retryChannel = rch

	for _, consumer := range cr.consumers {
		if err := consumer.Setup(ch, rch); err != nil {
...
func (r *Rabbit) Setup(consumerRunner *ConsumerRunner) error {
	...
	if err := consumerRunner.Setup(r.CH, r.RCH); err != nil {
		return err
	}
...
func (r *Rabbit) setupRetryChannel() error {
	ch, err := r.conn.Channel()
	if err != nil {
		return err
	}
	...
	if err := r.RCH.Qos(8, 0, false); err != nil {
		return err
	}
