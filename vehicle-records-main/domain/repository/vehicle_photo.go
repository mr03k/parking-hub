...
// BulkDelete deletes multiple citizen vehicle photos by their IDs
func (r *CitizenVehiclePhotoRepository) BulkDelete(ctx context.Context, ids []string) error {
	if len(ids) == 0 {
		return nil
	}

	result := r.DB.DB.WithContext(ctx).Delete(&entity.CitizenVehiclePhoto{}, ids)
	if result.Error != nil {
		return fmt.Errorf("failed to bulk delete citizen vehicle photos: %w", result.Error)
	}
