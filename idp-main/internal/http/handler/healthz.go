...
// rediness
func (s *HealthzHandler) HealthzLivenessOAPI(op openapi.OperationContext) {
...
// rediness
func (s *HealthzHandler) HealthzReadinessOAPI(op openapi.OperationContext) {
	op.SetSummary("Healthz Readiness")
	op.AddRespStructure(new(response.Response[string]))
	op.AddRespStructure(new(response.Response[string]), openapi.WithHTTPStatus(http.StatusInternalServerError))
}

func (s *HealthzHandler) OpenApiSpec(api OAPI) {
	healthzTag := WithTags("Healthz(Internal)")

	api.Register(http.MethodGet, "/healthz/liveness", s.HealthzLivenessOAPI, healthzTag)
	api.Register(http.MethodGet, "/healthz/readiness", s.HealthzReadinessOAPI, healthzTag)
}
